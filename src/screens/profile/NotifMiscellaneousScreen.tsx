...
type MiscType = typeof MISC_TYPES[number];
...
// ─── Misc Detail Sheet ─────────────────────────────────────────────────────────
function MiscDetailSheet({
    selectedType,
    settings,
    onUpdate,
    onClose,
}: {
    selectedType: MiscType | null;
    settings: TypeSettings;
    onUpdate: (field: 'system' | 'email', value: boolean) => void;
    onClose: () => void;
}): React.ReactElement {
    const footer = (
        <TouchableOpacity style={s.actionBtn} onPress={onClose} activeOpacity={0.85} testID="misc-update-settings">
        ...
        <BottomSheet visible={selectedType !== null} onClose={onClose} showBackButton footer={footer} maxHeight="90%">
            {selectedType && (
                <View style={detailS.content}>
                    ...
                    <View style={detailS.row}>
                        <View style={detailS.rowText}>
                            <Text style={detailS.rowLabel}>Notify by Email</Text>
                            <Text style={detailS.rowSubtitle}>Send to linked email</Text>
                        </View>
                        <Switch
...
const defaultSettings = (): Record<string, TypeSettings> =>
    Object.fromEntries(MISC_TYPES.map((t) => [t, { system: true, email: false }]));

export default function NotifMiscellaneousScreen(): React.ReactElement {
    const navigation = useNavigation<Nav>();

    const [isEditing, setIsEditing] = useState(false);
    const [typeSettings, setTypeSettings] = useState<Record<string, TypeSettings>>(defaultSettings);
    const [selectedType, setSelectedType] = useState<MiscType | null>(null);
    ...
    const renderViewMode = () => (
        <>
            <ScrollView style={s.flex} contentContainerStyle={s.viewScroll} showsVerticalScrollIndicator={false}>
                <Text style={s.sectionSubtitle}>Recent activities and pending tasks</Text>

                {MISC_TYPES.map((type, idx) => (
                    <View key={type}>
                        <View style={s.viewRow}>
                            <Text style={s.viewRowLabel}>{type}</Text>
                            <Text style={s.viewRowTag}>{getTypeLabel(typeSettings[type]).toUpperCase()}</Text>
                        </View>
                        {idx < MISC_TYPES.length - 1 && <View style={s.divider} />}
                    </View>
                ))}
            </ScrollView>

            <View style={s.footer}>
                <TouchableOpacity style={s.actionBtn} onPress={() => setIsEditing(true)} activeOpacity={0.85} testID="misc-edit">
    ...
    const renderEditMode = () => (
        <>
            ...
            <View style={s.footer}>
                <TouchableOpacity style={s.actionBtn} onPress={() => setIsEditing(false)} activeOpacity={0.85} testID="misc-update">
                    <Text style={s.actionBtnText}>Update Settings</Text>
                </TouchableOpacity>
            </View>
        </>
    ...
    return (
        <SafeAreaView style={s.safe} edges={['top']}>
            {/* Header — stays "Miscellaneous" in both modes */}
            <View style={s.header}>
                <TouchableOpacity
                    style={s.backBtn}
                    onPress={() => {
                ...
                <Text style={s.headerTitle}>Miscellaneous</Text>
...
const s = StyleSheet.create({
    ...
    flex: { flex: 1 },
    viewScroll: { paddingHorizontal: spacing.base, paddingTop: spacing.base },
    ...
    viewRowTag: { ...typography.bodySm, color: colors.textPrimary, fontWeight: '700' },
